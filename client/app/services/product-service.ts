...
//this is needed becuase the model defines timestamp as a string so that filter works
//but this returns timestamp as a Date
    ...
    getReviewsForProduct(productId: number): Observable<Review[]> {
        return this.http.get(`/products/${productId}/reviews`)
            ...
            .map(reviews => reviews.map(
               (review: any) => new Review(review.id, review.productId, new Date(review.timestamp), review.user, review.rating, review.comment)));
    }
